...
def _mesh_ao_original_state(obj, modifier, create=False):
    """Store modifier visibility on its owning Object for Blender 5.2.

    NodesModifier no longer supports arbitrary IDProperties in Blender 5.2,
    while Object remains an ID datablock and can safely hold the small state map.
    """
    states = obj.get(MESH_AO_ORIGINAL_VIEWPORT_PROP)
    if states is None and create:
        obj[MESH_AO_ORIGINAL_VIEWPORT_PROP] = {}
        states = obj[MESH_AO_ORIGINAL_VIEWPORT_PROP]
    if states is None:
        return None

    if modifier.name not in states and create:
        states[modifier.name] = bool(modifier.show_viewport)
    return bool(states[modifier.name]) if modifier.name in states else None


def _forget_mesh_ao_original_state(obj, modifier):
    states = obj.get(MESH_AO_ORIGINAL_VIEWPORT_PROP)
    if states is None or modifier.name not in states:
        return
    del states[modifier.name]
    if not states:
        del obj[MESH_AO_ORIGINAL_VIEWPORT_PROP]
...
def _set_mesh_ao_viewport_enabled(enabled):
    ...
    for obj in objects:
        for modifier in obj.modifiers:
            ...
            original = _mesh_ao_original_state(obj, modifier, create=True)
            ...
            if not original and modifier.show_render:
                original = True
                obj[MESH_AO_ORIGINAL_VIEWPORT_PROP][modifier.name] = True
...
def _restore_mesh_ao_viewport_states():
    ...
    for obj in objects:
        for modifier in obj.modifiers:
            if not _is_mesh_ao_modifier(modifier):
                continue
            original = _mesh_ao_original_state(obj, modifier)
            if original is None:
                continue
            ...
            if modifier.show_viewport != original:
                ...
                obj.update_tag()
                changed += 1
            _forget_mesh_ao_original_state(obj, modifier)
    return changed
