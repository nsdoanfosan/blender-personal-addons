...
bl_info = {
    ...
    "author": "Bart Crouch, Alexander Nedovizin, Paul Kotelevets "
              "(concept design), Adrian Rutkowski, JoseConseco",
    "version": (1, 8, 4),
    "blender": (2, 80, 0),
    "location": "Editmode > F",
    "warning": "",
    "description": "Fixed version of the F2 addon - it won't crash blender when using F2 with DrawXray addon",
...
def quad_from_edge(bm, edge_sel, context, event):
    ...
    all_edges = []
    for i in range(2):
        vertex = edge_sel.verts[i]
        filtered_edges = [
            edge for edge in vertex.link_edges
            if len(edge.link_faces) < 2 and edge != edge_sel and
               not any(face in edge_sel.link_faces for face in edge.link_faces)
        ]
        all_edges.append(filtered_edges)
